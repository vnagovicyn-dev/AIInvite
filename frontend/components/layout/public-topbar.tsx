    ...
    <header className="sticky top-0 z-40 border-b border-white/70 bg-background/80 backdrop-blur">
      <div className="mx-auto flex min-h-[76px] w-full max-w-7xl items-center justify-between gap-4 px-4 py-3 sm:px-6 lg:px-8">
        <div className="flex items-center gap-4">
          ...
          <div className="hidden rounded-full border bg-white/72 px-3 py-1.5 text-xs font-semibold uppercase tracking-[0.16em] text-muted-foreground xl:block">
            Онлайн-приглашения
          </div>
        </div>
        ...
        <details className="relative lg:hidden">
          <summary className="flex cursor-pointer list-none items-center rounded-full border bg-white/86 px-4 py-2 text-sm font-medium text-foreground">
            Меню
          </summary>
...
function MobileNavLink({
  ...
  return (
    <Link
      href={href}
      className="rounded-2xl px-3 py-2.5 text-foreground transition hover:bg-secondary/65"
    >
      {children}
    </Link>
  );
